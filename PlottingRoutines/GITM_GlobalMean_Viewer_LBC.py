...
index1 = 3
LBCMask = 10
alts = gdata['Altitude' ][0     ,0     ,0:LBCMask]/1000.0    # Alts in km
...
Temperature = gdata['Temperature'   ][nGCs:OrignLons-nGCs,nGCs:OrignLats-nGCs,:nAlts]   # Pull out Temperature
...
SumLonV    = np.sum(V          , axis=0)/nLons   # Our Zonal Mean
SumLonU    = np.sum(U          , axis=0)/nLons   # Our Zonal Mean
...
GMVelNorth = np.zeros(nAlts)
GMVelEast = np.zeros(nAlts)



for iAlt in range(nAlts):
#    LatWeightedTemp[0:nLats,iAlt] = sinlats[0:nLats]*SumLonTemp[0:nLats,iAlt]
    ...
    LatWeightedVar = sinlats[0:nLats]*SumLonV[0:nLats,iAlt]
    GMVelNorth[iAlt] = np.sum(LatWeightedVar)/sumsinlats
    LatWeightedVar = sinlats[0:nLats]*SumLonU[0:nLats,iAlt]
    GMVelEast[iAlt] = np.sum(LatWeightedVar)/sumsinlats
...
title_text_size = 28
tick_font_size = 13
TitleString = 'Global Mean Temperature (K) ' 
...
TitleString = 'Global Mean Radial Velocity (m/s) ' 
...
TitleString = 'Global Mean Northern Velocity (m/s) ' 

fig, ax = plt.subplots(nrows=1,ncols=1,figsize=(8.5,11),sharex=True)
line2, = ax.plot(GMVelNorth, alts)
ax.tick_params(axis='both',labelsize=tick_font_size)    
ax.set_title(TitleString,size = title_text_size)   
ax.set_xlabel('Velocity (m/s)',size = axis_text_size)
ax.set_ylabel('Altitude (km)',size = axis_text_size)
...
TitleString = 'Global Mean Eastern Velocity (m/s) ' 

fig, ax = plt.subplots(nrows=1,ncols=1,figsize=(8.5,11),sharex=True)
line2, = ax.plot(GMVelEast, alts)
ax.tick_params(axis='both',labelsize=tick_font_size)    
ax.set_title(TitleString,size = title_text_size)   
ax.set_xlabel('Velocity (m/s)',size = axis_text_size)
ax.set_ylabel('Altitude (km)',size = axis_text_size)
